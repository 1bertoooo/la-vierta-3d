  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const r = await fetch("/api/cadastrar", {
        ...
        body: JSON.stringify({ nick: nick.trim().toLowerCase(), password: senha }),
      });
      const data = await r.json();
      if (!r.ok || !data.ok) throw new Error(data.error || "Erro no cadastro");
      router.push("/login?nick=" + encodeURIComponent(nick.trim().toLowerCase()));
    } catch (err) {
    ...
    <main className="h-screen w-screen flex items-center justify-center px-6">
      ...
      >
        ...
        <p className="text-[var(--color-pergaminho-velho)] text-sm mb-6">
          Escolha um nick. Não tem confirmação por email — vamos direto.
        </p>
        ...
        <p className="mt-4 text-xs text-[var(--color-pergaminho-velho)] text-center">
          Já tem conta?{" "}
          <Link href="/login" className="text-[var(--color-dourado)] hover:underline">
            Login
          </Link>
        </p>
      </form>
